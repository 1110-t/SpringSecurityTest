...
### ログイン画面を表示
実行したら、早速、"http://127.0.0.1:8080/login"にアクセスしてみてください。以下のような画面にアクセスできます。
Usernameにuserと入力し、Passwordに先ほどの画面でコピーしたパスワードを入力してください。そうすると、"http://127.0.0.1:8080"にアクセスされると思います。ログインが完了することを確認できました。ここで特にコントローラーなどで何も設定していない場合は、Whitelabel Error Pageが表示されます。
